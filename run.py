...
const ITEMS = {
  contracts: [
    ...
    { id:2, title:"מאפ״ת — הגנת נקודה נגד נחילים", source:"מלמ״ב / מאפ״ת", url:"https://www.mod.gov.il", country:"ישראל", flag:"🇮🇱", budget:"₪12M", deadline:"20.04.2025", urgency:"critical", fitScore:98, tag:"מלמ״ב", why:"Sky Fort + Aerosentry — stack מלא DTID. אין מתחרה ישראלי.", status:"בבדיקה", bookmarked:true },
...
function Tag({label,color="#475569"}){
  return <span style={{fontSize:"10px",fontWeight:700,padding:"2px 7px",borderRadius:"4px",border:`1px solid ${color}50`,background:`${color}15`,color,fontFamily:"monospace"}}>{label}</span>;
}
...
function Card({item,borderColor,children}){
  ...
  return (
    <div onClick={()=>item.url&&window.open(item.url,"_blank")}
      ...
      {item.url&&<div style={{position:"absolute",top:9,left:11,fontSize:"9px",color:h?"#60a5fa":"#1e3a5f",fontFamily:"monospace",transition:"color .2s"}}>↗ פתח מקור</div>}
      {children}
    </div>
  );
}
                ...
                <div key={item.id} onClick={()=>item.url&&window.open(item.url,"_blank")}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:6,marginTop:3,flexWrap:"wrap",alignItems:"center"}}>
                      {item.budget&&<span style={{fontSize:"10px",color:"#22c55e",fontFamily:"monospace",fontWeight:700}}>{item.budget}</span>}
                      {item.prize&&<span style={{fontSize:"10px",color:"#22c55e",fontFamily:"monospace",fontWeight:700}}>{item.prize}</span>}
                      {item.deadline&&<span style={{fontSize:"10px",color:"#f87171",fontFamily:"monospace"}}>{item.deadline}</span>}
                      {item.fitScore&&<span style={{fontSize:"10px",color:fitCol(item.fitScore),fontFamily:"monospace"}}>fit {item.fitScore}</span>}
  ...
  return (
    <Card item={item} borderColor={u.border}>
      <div style={{display:"flex",justifyContent:"space-between",gap:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",width:42,height:42,borderRadius:"50%",flexShrink:0,border:`2px solid ${fitCol(item.fitScore)}`,background:`${fitCol(item.fitScore)}15`,fontSize:"12px",fontWeight:800,color:fitCol(item.fitScore),fontFamily:"monospace"}}>{item.fitScore}</div>
      </div>
      <WhyBox text={item.why}/>
      <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
        {item.budget&&<div><div style={{fontSize:"9px",color:"#475569",marginBottom:1}}>תקציב</div><div style={{fontSize:"13px",fontWeight:700,color:"#38bdf8",fontFamily:"monospace"}}>{item.budget}</div></div>}
        <div><div style={{fontSize:"9px",color:"#475569",marginBottom:1}}>דדליין</div><div style={{fontSize:"12px",fontWeight:700,color:item.urgency==="critical"?"#f87171":"#94a3b8",fontFamily:"monospace"}}>{item.deadline}</div></div>
      </div>
  ...
  return (
    <Card item={item} borderColor={u.border}>
      ...
      <div style={{display:"flex",gap:18}}>
        <div><div style={{fontSize:"9px",color:"#475569",marginBottom:1}}>פרס</div><div style={{fontSize:"14px",fontWeight:700,color:"#4ade80",fontFamily:"monospace"}}>{item.prize}</div></div>
        <div><div style={{fontSize:"9px",color:"#475569",marginBottom:1}}>דדליין</div><div style={{fontSize:"12px",fontWeight:700,color:"#94a3b8",fontFamily:"monospace"}}>{item.deadline}</div></div>
      </div>
    </Card>
  );
}
...
const GEO_TRENDS = [
  ...
  },
  {
    ...
    whitespace:"מאפ״ת ומלמ״ב — הזדמנויות בית. ניסיון מלחמה = TRL טבעי. הכל מוכן.",
    keyBuyers:["מאפ״ת","מלמ״ב","צה״ל","מודיעין"],
    trend:"Post-Oct 7: תקציב ביטחון הוכפל. Counter-UAS ו-ISR בעדיפות עליונה.",
    signals:["מאפ״ת — קולות קוראים שוטפים","מלמ״ב — אנרגיה וC-UAS","IDF — robotic platoon program"],
  },
  {
...
function MiniBar({val, color="#3b82f6", label}){
  return (
    <div style={{display:"flex",alignItems:"center",gap:6}}>
      ...
      <span style={{fontSize:"9px",color:"#475569",fontFamily:"monospace",width:28}}>{val}%</span>
    </div>
  );
}
    ...
    <div onClick={onToggle} style={{background:"#0f172a",border:`1px solid ${t.recColor}20`,borderTop:`3px solid ${t.recColor}`,borderRadius:"10px",padding:"16px",cursor:"pointer",transition:"box-shadow .2s",boxShadow:expanded?`0 4px 20px ${t.recColor}18`:"none"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
            <span style={{fontSize:"11px",color:"#22c55e",fontFamily:"monospace",fontWeight:700}}>{t.market}</span>
            <span style={{fontSize:"11px",color:"#38bdf8",fontFamily:"monospace"}}>{t.cagr}</span>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
          ...
          <div style={{width:34,height:34,borderRadius:"50%",border:`2px solid ${fc}`,background:`${fc}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"12px",fontWeight:800,color:fc,fontFamily:"monospace"}}>{t.givonFit}</div>
        </div>
      </div>
      ...
      {t.givonAssets.length>0&&(
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:6}}>
          {t.givonAssets.map(a=><span key={a} style={{fontSize:"9px",background:"#1e3a5f",border:"1px solid #3b82f625",color:"#60a5fa",padding:"2px 7px",borderRadius:"4px",fontFamily:"monospace"}}>{a}</span>)}
        </div>
      )}
  ...
  return (
    <div onClick={onToggle} style={{background:"#0f172a",border:"1px solid #1e293b",borderTop:`3px solid ${g.accessColor}`,borderRadius:"10px",padding:"16px",cursor:"pointer",transition:"box-shadow .2s",boxShadow:expanded?`0 4px 20px ${g.accessColor}18`:"none"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <span style={{fontSize:"12px",color:"#22c55e",fontFamily:"monospace",fontWeight:700}}>{g.budget}</span>
      ...
      <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
        {g.hotDomains.map(d=><span key={d} style={{fontSize:"9px",background:"#1e293b",color:"#94a3b8",padding:"2px 7px",borderRadius:"4px",fontFamily:"monospace"}}>{d}</span>)}
      </div>
          ...
          {topPicks.map(t=>(
            <div key={t.id} onClick={()=>{setTab("tech");toggle(t.id);}}
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:"10px",color:"#22c55e",fontFamily:"monospace"}}>{t.market} · {t.cagr}</div>
              </div>
              <div style={{width:28,height:28,borderRadius:"50%",border:"2px solid #22c55e",background:"#22c55e15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"10px",fontWeight:800,color:"#22c55e",fontFamily:"monospace"}}>{t.givonFit}</div>
            </div>
          ))}
          {/* Hot geo strip */}
          {hotGeo.slice(0,2).map(g=>(
            <div key={g.id} onClick={()=>{setTab("geo");toggle(g.id);}}
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:"10px",color:"#fb923c",fontFamily:"monospace"}}>{g.budget} · {g.momentum}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{marginBottom:6}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
        ...
        <span style={{fontSize:"10px",color:"#94a3b8",fontFamily:"monospace"}}>{val}%</span>
      </div>
    ...
    <div onClick={onToggle} style={{background:"#0f172a",border:`1px solid ${t.recColor}20`,borderTop:`3px solid ${t.recColor}`,borderRadius:"10px",padding:"18px",cursor:"pointer",transition:"box-shadow .2s",boxShadow:expanded?`0 4px 24px ${t.recColor}15`:"none"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <span style={{fontSize:"11px",color:"#22c55e",fontFamily:"monospace",fontWeight:700}}>{t.market}</span>
            <span style={{fontSize:"11px",color:"#38bdf8",fontFamily:"monospace"}}>{t.cagr} CAGR</span>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            ...
            <div style={{width:32,height:32,borderRadius:"50%",border:`2px solid ${fitColor}`,background:`${fitColor}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:800,color:fitColor,fontFamily:"monospace"}}>{t.givonFit}</div>
          </div>
        </div>
      </div>
...
function TrendsView() {
  ...
  const recFilters = [
    { id:"all", label:"הכל" },
    { id:"להיכנס", label:"להיכנס", color:"#22c55e" },
    { id:"לחקור", label:"לחקור", color:"#f59e0b" },
    { id:"לעקוב", label:"לעקוב", color:"#3b82f6" },
    { id:"לא עכשיו", label:"לא עכשיו", color:"#ef4444" },
  ];

  const filtered = filter==="all" ? TRENDS : TRENDS.filter(t=>t.recommendation===filter);

  // Top 3 recommended
  const topPicks = TRENDS.filter(t=>t.recommendation==="להיכנס").sort((a,b)=>b.givonFit-a.givonFit).slice(0,3);
  ...
  return (
    <div style={{maxWidth:1300,margin:"0 auto",padding:"24px"}}>
      ...
      <div style={{marginBottom:20}}>
        <div style={{fontSize:"20px",fontWeight:800,color:"#f1f5f9",marginBottom:4}}>📈 מגמות אסטרטגיות</div>
        <div style={{fontSize:"12px",color:"#475569"}}>לאן זורם כסף הביטחון הגלובלי — וכיצד גבעון ממוקמת ביחס לכל תחום</div>
      </div>

      {/* Top picks */}
      <div style={{background:"linear-gradient(135deg,#0a1628,#0c1f35)",border:"1px solid #1e40af30",borderRadius:"10px",padding:"16px",marginBottom:20}}>
        <div style={{fontSize:"11px",color:"#60a5fa",fontWeight:700,marginBottom:10,letterSpacing:"0.06em"}}>⚡ TOP PICKS — תחומים שגבעון צריכה להיכנס עכשיו</div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          {topPicks.map(t=>(
            <div key={t.id} onClick={()=>{setFilter("all");setExpanded(expanded===t.id?null:t.id);}} style={{background:"#0f172a",border:`1px solid ${t.recColor}40`,borderRadius:"8px",padding:"10px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,flex:1,minWidth:200}}>
              <span style={{fontSize:16}}>{t.icon}</span>
              <div>
                <div style={{fontSize:"12px",fontWeight:700,color:"#f1f5f9"}}>{t.domain}</div>
                <div style={{display:"flex",gap:6,marginTop:2}}>
                  <span style={{fontSize:"10px",color:"#22c55e",fontFamily:"monospace"}}>{t.market}</span>
                  <span style={{fontSize:"10px",color:"#38bdf8",fontFamily:"monospace"}}>{t.cagr}</span>
                </div>
              </div>
              <div style={{marginRight:"auto",width:32,height:32,borderRadius:"50%",border:`2px solid ${t.givonFit>=85?"#22c55e":"#eab308"}`,background:`${t.givonFit>=85?"#22c55e":"#eab308"}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"11px",fontWeight:800,color:t.givonFit>=85?"#22c55e":"#eab308",fontFamily:"monospace"}}>{t.givonFit}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16}}>
        {recFilters.map(f=>(
          <button key={f.id} onClick={()=>setFilter(f.id)} style={{background:filter===f.id?(f.color?"transparent":"#1e3a5f"):"transparent",border:`1px solid ${filter===f.id?(f.color||"#3b82f6"):"#1e293b"}`,color:filter===f.id?(f.color||"#60a5fa"):"#475569",padding:"5px 12px",borderRadius:"6px",fontSize:"11px",fontWeight:filter===f.id?700:400,cursor:"pointer"}}>
            {f.label} {f.id!=="all"&&<span style={{fontFamily:"monospace",marginRight:3}}>{TRENDS.filter(t=>t.recommendation===f.id).length}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(380px,1fr))",gap:14}}>
        {filtered.map(t=>(
          <TrendCard key={t.id} t={t} expanded={expanded===t.id} onToggle={()=>setExpanded(expanded===t.id?null:t.id)}/>
        ))}
      </div>
    </div>
  );
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:6,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:"10px",fontWeight:700,padding:"2px 7px",borderRadius:"4px",border:`1px solid ${t.color}50`,background:`${t.color}15`,color:t.color,fontFamily:"monospace"}}>{t.icon} {t.label}</span>
            <span style={{fontSize:"10px",fontWeight:700,padding:"2px 7px",borderRadius:"4px",border:`1px solid ${sc}50`,background:`${sc}15`,color:sc,fontFamily:"monospace"}}>{p.status}</span>
          </div>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,marginRight:8,flexShrink:0}}>
          <div style={{width:38,height:38,borderRadius:"50%",border:`2px solid ${fc}`,background:`${fc}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"12px",fontWeight:800,color:fc,fontFamily:"monospace"}}>{p.fit}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:20}}>
        {[
          ...
          <div key={label} style={{background:"#0a0f1e",border:`1px solid ${color}20`,borderTop:`2px solid ${color}`,borderRadius:"8px",padding:"12px",textAlign:"center"}}>
            <div style={{fontSize:"22px",fontWeight:800,color,fontFamily:"monospace",lineHeight:1}}>{val}</div>
...
export default function App(){
  ...
  return (
    <div style={{minHeight:"100vh",background:"#020617",color:"#e2e8f0",fontFamily:"'Segoe UI',Tahoma,sans-serif",direction:"rtl"}}>
      ...
      {/* ── Header ── */}
      <div style={{background:"#0a0f1e",borderBottom:"1px solid #1e293b",padding:"0 24px",position:"sticky",top:0,zIndex:50}}>
        <div style={{maxWidth:1300,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            {[["briefing","📊 בריפינג יומי"],["catalog","📋 קטלוג"],["trends","📈 מגמות"],["partnerships","🎯 שותפויות"]].map(([v,label])=>(
              <button key={v} onClick={()=>setView(v)} style={{background:"transparent",border:"none",borderBottom:view===v?"2px solid #3b82f6":"2px solid transparent",color:view===v?"#60a5fa":"#475569",padding:"0 4px",height:60,fontSize:"13px",fontWeight:view===v?700:400,cursor:"pointer",transition:"all .15s"}}>{label}</button>
            ))}
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:20}}>
            {[
              ...
              <div key={label} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1}}>
                <div style={{fontSize:"18px",fontWeight:800,color,fontFamily:"monospace",lineHeight:1}}>{val}</div>
            ...
            {CATEGORIES.map(cat=>{
              ...
              return (
                <button key={cat.id} onClick={()=>setActiveCat(cat.id)} style={{background:on?"#1e3a5f":"#0a0f1e",border:`1px solid ${on?"#3b82f6":"#1e293b"}`,color:on?"#60a5fa":"#475569",padding:"7px 14px",borderRadius:"8px",fontSize:"12px",fontWeight:on?700:400,cursor:"pointer",display:"flex",alignItems:"center",gap:5,transition:"all .15s"}}>
                  ...
                  <span style={{background:on?"#1d3a60":"#1e293b",color:on?"#60a5fa":"#475569",fontSize:"10px",padding:"1px 5px",borderRadius:"8px",fontFamily:"monospace"}}>{count}</span>
                </button>
              );
            })}
          </div>
          ...
          <div style={{display:"flex",gap:5,marginBottom:20,alignItems:"center"}}>
            ...
            {ALL_COUNTRIES.map(({flag,label})=>(
              <button key={flag} onClick={()=>setCountryFilter(countryFilter===label?null:label)}
                ...
                {flag}
                {countryFilter===label&&<span style={{fontSize:"9px",color:"#60a5fa",fontFamily:"monospace"}}>{label}</span>}
              </button>
            ))}
